...
There are basically no rules right now when it comes to writing code using LLMs.

There are principles, anecdotal findings, general trends, repeatable patterns, and some "standards" like [AGENTS.md](https://agents.md/) and [Agent Skills](https://agentskills.io/).

If you actually read those websites, you'll find a lot more words than necessary to explain what these concepts are:
- Markdown files
- Markdown files that _most_ "agentic harnesses" look for

In general, `AGENTS.md` gets loaded into the context window every time. Agent Skills (`.agents/`) have metadata that is loaded into the context window every time, and the model can load the full context when it "thinks" the added context would be useful (progressive disclosure).
...
`CLAUDE.md` (and its friend `.claude/`) are dumb.
...
Right now, in the Remix 3 template (still in beta), [we just have `AGENTS.md` and `./agents`](https://github.com/remix-run/remix/tree/e1d20c0348a010a94233d213bf1df17a699c7544/template). We could very easily include a symlink `CLAUDE.md` or a minimal one like in the React Router repo, but that's dumb.
